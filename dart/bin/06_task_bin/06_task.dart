import 'package:surf_dart_courses/06_task/06_task.dart';
...
void main() {
  final List<Product> products = [
    ...
    Product(
        id: 3, category: 'молоко', name: 'Полосатый кот', price: 50, count: 53),
  ...
  List<Product> filteredProductsByPrice =
      applyFilter(products, priceLessThan200);
  List<Product> filteredProductsByCount = applyFilter(products, filterByCount);
  List<Product> filteredProductsByCategory =
      applyFilter(products, filterByCategory);
  filteredProductsByPrice.forEach((product) {
    print(
        "${product.id}  ${product.category} ${product.name}  ${product.price} рублей ${product.count} шт");
  });
}
